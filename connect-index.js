 ...
 * kindly update the index file of the gi-connect package inside the node_module
 folder with the content of the this file to use the current infura gateway */
...
// Added manual ipfs abi this for testing
const IPFSabi = {
    registryAbi: 'QmdwCLu6VHmcEPkaNNNb4RLMCUeanzksNjAfk3to2hL7xN',
    registryControllerABI: 'QmNZSHvUYaL3YV6Ybuhuj4g8bDZqar25usttGTaCYzoy38'
  }
...
gif.Instance = class Instance {
   ...
   * @returns {Promise<*[]|*>}
   */
  async getAbi(addr, augmentAbi= false) {
    try {
      ...
      // Added this line for testing
        const ipfsAbiPath = augmentAbi ? IPFSabi.registryControllerABI : IPFSabi.registryAbi;
        const gatewayLink = `https://infura-ipfs.io/ipfs/${ipfsAbiPath}`;
  ...
  async augmentAbi(contractAbi, controllerAddress) {
    if (controllerAddress !== '0x0000000000000000000000000000000000000000') {
      ...
      const controllerAbi = await this.getAbi(controllerAddress, true);
